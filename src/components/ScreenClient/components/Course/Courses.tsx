...
const sortCourse = (course: DataCourse []) => {
  return course.sort((course, otherCourse) => course.subject.localeCompare(otherCourse.subject))
}

function Courses() {
  const courseMessages = useSocketStore(state => state.getCoursesMessages())
  const courseMessagesCarousel = carouselTableArray(sortCourse(courseMessages), 11)
