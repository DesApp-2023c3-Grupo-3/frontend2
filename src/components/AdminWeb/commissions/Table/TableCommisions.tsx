...
function TableCommisions({ commisions }: { commisions: Commission[] }) {
  return (
    <table className="table-auto border-collapse overflow-hidden rounded-tl-[20px] rounded-tr-[20px] ml-10 mt-10 font-[500]">
      ...
      <tbody className="text-[20px] font[500]">
        {commisions.map((data, index) => (
          <TableRow key={data.id} commision={data} index={index} />
        ))}
      </tbody>
    </table>
  );
...
export default TableCommisions;
