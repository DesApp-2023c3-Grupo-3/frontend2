...
function Sectores({ selectedRole, onSelectedRoleChange }: RolesProps) {
  ...
  return (
    <div className={`${isMobile && 'w-[90%]'} h-[50px]`}>
      <Listbox value={selectedRole} onChange={onSelectedRoleChange}>
        <Listbox.Button
          className={`
              ${isMobile ? 'w-[90vw]' : 'w-full'}
              bg-default-100 transition-all hover:bg-default-200 text-[17px] font-[400] tracking-[-0.4px] rounded-[30px] flex h-[50px] px-[40px] py-[12px] items-center
              `}
        >
          <div className="mr-2 ml-[-25px]">
            <svg
